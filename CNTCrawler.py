...
class CNTCrawler():
    ...
    DATATUPLE = namedtuple('Article', ['category', 'datetime', 'title', 'content', 'url'])
    ...
    def parser(self, url):
        ...
        if req.status_code == 200:  # 確認網站正確連接
            ...
            time.sleep(1)  # 間格時間,防止對網站query過快
            return self.DATATUPLE(category, datetime, title, content, url)
        else:
            return False
...
if __name__ == '__main__':
    ...
    article_url = crawler.get_url(maxpage=100)
